...
public:
  static void on_global_setup(const global_setup_context &ctx) {
    ...
    try {
      auto duck = ctx.get_suite_with_alert<DuckSuite1>(
          kDuckSuite1, kDuckSuiteVersion1,
  ...
  static void on_do_dialog(const context &ctx) {
    ctx.set_dialog_response("Hello from AETK Checkout!\rThis is a "
                            "platform-independent dialog response.");
  }

  static void on_render(const render_context &ctx) {
    auto output = ctx.output();
    // Lookup using custom string key
    int32_t offset_frames = ctx.int_val("frame_offset_str");
  ...
  static void on_pre_render(const pre_render_context &ctx) {
    ...
    int32_t offset_frames = ctx.int_val(checkout_keys::frame_offset);

    // Checkout the input layer normally (ID 0)
    ctx.checkout_layer(0, 0);

    // Checkout the selected layer at the specified offset (ID 1) using custom
    // string key
