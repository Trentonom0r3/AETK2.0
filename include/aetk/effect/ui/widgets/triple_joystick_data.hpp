...
/**
 * @brief Data structure storing three independent 2D joystick coordinates for SWARM modes.
 */
struct triple_joystick_data {
    joystick_data silhouette;
    joystick_data hsv;
    joystick_data yolo;

    bool operator==(const triple_joystick_data& other) const {
...
template <>
struct arb_traits<ui::triple_joystick_data> {
    using T = ui::triple_joystick_data;
    ...
    static void print(const T* ptr, char* str, size_t max_len) {
        if (max_len > 0) {
            aetk::core::c_snprintf(str, max_len, "[Sil: (%.2f, %.2f), Hsv: (%.2f, %.2f), Yolo: (%.2f, %.2f)]",
    ...
    static bool scan(T* ptr, const char* str) {
        ...
        if (aetk::core::c_sscanf(str, "[Sil: (%f, %f), Hsv: (%f, %f), Yolo: (%f, %f)]",
                       &sil_x, &sil_y, &hsv_x, &hsv_y, &yolo_x, &yolo_y) == 6) {
